...
-- Fri 16 May 2014 01:23:09 PM BRT
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `osuply_app`.`itensCompra` (`id` INT, `produto` INT, `quantidade` INT, `comprador` INT, `dtAlteracao` INT, `dtCriacao` INT, `usrCriou` INT, `usrAlterou` INT, `descricao` INT, `pessoa` INT, `tipo` INT, `subcategoria` INT, `Categoria` INT);
...
VIEW `itensCompra` AS
    select 
        ...
        `FP`.`comprador` AS `comprador`,
        `FP`.`dtAlteracao` AS `dtAlteracao`,
        `FP`.`dtCriacao` AS `dtCriacao`,
        `FP`.`usrCriou` AS `usrCriou`,
        `FP`.`usrAlterou` AS `usrAlterou`,
    ...
    from
        (((((`compradorProduto` `FP`
        join `produto` `P` ON ((`FP`.`produto` = `P`.`id`)))
        join `preferenciasCompra` `PC` ON ((`P`.`categoria` = `PC`.`subcategoria`)))
        join `categoria` `C` ON ((`C`.`id` = `PC`.`categoria`)))
        join `tipos` `T` ON ((`T`.`id` = `PC`.`tipo`)))
        join `subCategoria` `SC` ON ((`SC`.`id` = `PC`.`subcategoria`)));
