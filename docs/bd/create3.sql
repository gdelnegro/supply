...
-- Thu 08 May 2014 05:58:17 PM BRT
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `osuply_app`.`preferenciasDeVenda` (`pessoa` INT, `'Tipo'` INT, `'idTipo'` INT, `'Categoria'` INT, `'idCategoria'` INT, `'SubCategoria'` INT, `'idSubCategoria'` INT);

-- -----------------------------------------------------
-- Placeholder table for view `osuply_app`.`preferenciasDeCompra`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `osuply_app`.`preferenciasDeCompra` (`pessoa` INT, `'Tipo'` INT, `'idTipo'` INT, `'Categoria'` INT, `'idCategoria'` INT, `'SubCategoria'` INT, `'idSubCategoria'` INT);

-- -----------------------------------------------------
-- Placeholder table for view `osuply_app`.`itensVende`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `osuply_app`.`itensVende` (`"codigoRegistro"` INT, `'FullPath'` INT, `quantidade` INT, `precoitem` INT, `Fornecedor` INT, `Codigo` INT, `descricao` INT);

-- -----------------------------------------------------
-- Placeholder table for view `osuply_app`.`itensCompra`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `osuply_app`.`itensCompra` (`id` INT, `produto` INT, `quantidade` INT, `precoItem` INT, `fornecedor` INT, `codigo` INT, `dtAlteracao` INT, `dtCriacao` INT, `usrCriou` INT, `usrAlterou` INT, `descricao` INT, `pessoa` INT, `tipo` INT, `subcategoria` INT, `'Categoria'` INT);
...
-- -----------------------------------------------------
-- View `osuply_app`.`itensVende`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `osuply_app`.`itensVende`;
USE `osuply_app`;
CREATE  OR REPLACE VIEW `itensVende` AS

SELECT 
FP.id as "codigoRegistro",
CONCAT(T.descricao,'-',C.descricao,'-',SC.descricao) AS 'FullPath',
FP.quantidade, FP.precoitem, FP.Fornecedor, FP.Codigo,
P.descricao
FROM fornecedorProduto as FP
INNER JOIN produto AS P ON FP.produto = P.id
INNER JOIN subCategoria as SC ON P.categoria = SC.id
INNER JOIN categoria AS C ON SC.categoria = C.id
INNER JOIN tipos AS T ON C.tipo = T.id;

-- -----------------------------------------------------
-- View `osuply_app`.`itensCompra`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `osuply_app`.`itensCompra`;
USE `osuply_app`;
CREATE  OR REPLACE VIEW `itensCompra` AS

SELECT FP.* , P.descricao, PC.pessoa, PC.tipo, PC.subcategoria, concat(T.descricao,'-',C.descricao,'-',SC.descricao) AS 'Categoria'
FROM fornecedorProduto as FP
INNER JOIN produto as P on FP.produto = P.id
INNER JOIN preferenciasCompra as PC ON P.categoria = PC.subcategoria
INNER JOIN categoria AS C ON C.id = PC.categoria
INNER JOIN tipos AS T ON T.id = PC.tipo
INNER JOIN subCategoria AS SC ON SC.id = PC.subcategoria;
