...
class Admin_BannerController extends Zend_Controller_Action
{
    ...
    {
        $anuncios = new Admin_Model_DbTable_Banners();
    ...
    {
        $form = new Admin_Form_Banner('new',$this->_usuario->id);
        
        if( $this->getRequest()->isPost() ) {
            $data = $this->getRequest()->getPost();
            //die(var_dump($data));
            
            $anuncio = new Admin_Model_DbTable_Banners();
            ...
            if( $this->getRequest()->isPost() ) {
                ...
                if ( $form->isValid($data) ){          
                    ...
                    foreach ($upload->getFileInfo() as $file => $info) {                                     
                        ...
                        $dir = APPLICATION_PATH.'/../public/images/banners/'.$titulo.'/';
                        if (!is_dir($dir)) {
                            mkdir($dir);         
                        }
                        $upload->addFilter('Rename', array( 'target' => APPLICATION_PATH.'/../public/images/banners/'.$titulo.'/'.$titulo.'.'.$extension,'overwrite' => true,));
                    }
                    try {
                        $upload->receive();
                    ...
                    $dados =array(
                        'descricao'  =>   'Banner'.$this->_getParam('sponsor'),
                        'titulo'      =>  $titulo.'.'.$extension,
                        'local'     =>  '/images/banners/'.$titulo.'/',
                        'link'      =>  $link,
                        'tipo' =>  '1'
                    );
